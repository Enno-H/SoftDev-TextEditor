...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		while(!queue.isEmpty()&&!visited.contains(word2)) {
			...
			List neighbors = nw.distanceOne(curr.getWord(),true);
			for(int i = 0;i < neighbors.size();i++) {
				if(!visited.contains(neighbors.get(i))) {
					curr.addChild((String)neighbors.get(i));
					visited.add((String)neighbors.get(i));
					WPTreeNode newNode = new WPTreeNode((String)neighbors.get(i),curr);
					queue.add(newNode);
					if(neighbors.get(i).equals(word2)) {
